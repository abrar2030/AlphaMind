...
async function connectToMarketData() {
  ...
  subscription.on('data', (tick) => {
    console.log(`${tick.ticker} @ ${tick.timestamp}: $${tick.last} (Vol: ${tick.volume})`);
...
class MovingAverageCrossover {
  ...
  checkForSignal(ticker) {
    ...
    if (shortMA > longMA && this.positions[ticker] !== 'LONG') {
      console.log(`BUY SIGNAL for ${ticker}: Short MA (${shortMA.toFixed(2)}) crossed above Long MA (${longMA.toFixed(2)})`);
    ...
    // Sell signal: short MA crosses below long MA
      ...
      console.log(`SELL SIGNAL for ${ticker}: Short MA (${shortMA.toFixed(2)}) crossed below Long MA (${longMA.toFixed(2)})`);
