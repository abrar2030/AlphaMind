...
class SentimentBasedStrategy:
    ...
    def calculate_signals(self, news_data, lookback_window=5):
        ...
        sentiment_scores = self.sentiment_analyzer.get_sentiment_score(news_data['text'].values)
        ...
        merged_data['sentiment_ma'] = merged_data['sentiment_score'].rolling(window=lookback_window).mean()
        
        # Generate signals based on sentiment momentum
        merged_data['sentiment_signal'] = np.where(
            merged_data['sentiment_ma'] > 0.3, 1,  # Strong positive sentiment
            np.where(merged_data['sentiment_ma'] < -0.3, -1, 0)  # Strong negative sentiment
        )
        ...
        merged_data['combined_signal'] = 0.5 * merged_data['sentiment_signal'] + 0.5 * merged_data['price_signal']
        
        # Final trading decision
        merged_data['position'] = np.where(
            merged_data['combined_signal'] > 0.25, 1,  # Long position
    ...
    def backtest(self, initial_capital=10000):
        ...
        backtest_data['strategy_return'] = backtest_data['position'].shift(1) * backtest_data['market_return']
        
        # Calculate cumulative returns
        backtest_data['cumulative_market_return'] = (1 + backtest_data['market_return']).cumprod()
        backtest_data['cumulative_strategy_return'] = (1 + backtest_data['strategy_return']).cumprod()
        
        # Calculate equity curves
        backtest_data['market_equity'] = initial_capital * backtest_data['cumulative_market_return']
        backtest_data['strategy_equity'] = initial_capital * backtest_data['cumulative_strategy_return']
    ...
    def plot_results(self, backtest_data):
        ...
        plt.plot(backtest_data['date'], backtest_data['strategy_equity'], label='Strategy')
        ...
        # Plot drawdowns
        plt.subplot(3, 1, 2)
        plt.plot(backtest_data['date'], backtest_data['market_drawdown'], label='Market')
        plt.plot(backtest_data['date'], backtest_data['strategy_drawdown'], label='Strategy')
        plt.title('Drawdowns')
        plt.legend()
        plt.grid(True)
        ...
        plt.subplot(3, 1, 3)
        plt.plot(backtest_data['date'], backtest_data['sentiment_ma'], label='Sentiment MA')
