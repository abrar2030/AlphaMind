...
import pandas as pd
import numpy as np
from sentinelhub import WmsRequest, SentinelAPI
...
class SatelliteFeatureExtractor:
    ...
    def _get_company_facilities(self, ticker):
        ...
        if ticker == "AAPL":
            return [
                {"name": "Apple Park", "coordinates": [37.3346, -122.0090, 37.3456, -121.9880]},
                {"name": "Foxconn Zhengzhou", "coordinates": [34.7500, 113.6250, 34.7700, 113.6550]}
            ]
        elif ticker == "TSLA":
            return [
                {"name": "Fremont Factory", "coordinates": [37.4920, -121.9465, 37.4970, -121.9365]},
                {"name": "Gigafactory Nevada", "coordinates": [39.5380, -119.4410, 39.5480, -119.4310]}
            ]
        else:
            return []
