...
def optimizer():
    ...
    opt = PortfolioOptimizer(n_assets=N_ASSETS, lookback_window=LOOKBACK_WINDOW, hidden_units=HIDDEN_UNITS)
...
def test_optimizer_init(optimizer):
    ...
    assert isinstance(optimizer.scaler, object) # StandardScaler is tricky to assert type directly
...
def test_build_model_structure(optimizer):
    ...
    assert model.input_shape[2] == (None, LOOKBACK_WINDOW, N_MACRO_FACTORS) # macro_input
...
def test_portfolio_loss_calculation():
    ...
    y_pred = tf.constant([[0.4, 0.3, 0.3], [0.5, 0.2, 0.3]], dtype=tf.float32) # Example weights
    ...
    expected_return_1 = 0.4 * 0.01 + 0.3 * (-0.005) + 0.3 * 0.02 # 0.004 - 0.0015 + 0.006 = 0.0085
    expected_return_2 = 0.5 * 0.005 + 0.2 * 0.01 + 0.3 * (-0.01) # 0.0025 + 0.002 - 0.003 = 0.0015
...
def test_preprocess_data_shapes(optimizer, sample_data):
    ...
    X_price, X_vol, X_macro, y = optimizer.preprocess_data(price_data, volatility_data, macro_data)
...
def test_optimize_portfolio_output(optimizer, sample_data):
    ...
    optimal_weights = optimizer.optimize_portfolio(recent_price, recent_vol, recent_macro, current_weights)
    
    assert optimal_weights.shape == (N_ASSETS,)
    assert np.isclose(np.sum(optimal_weights), 1.0) # Weights should sum to 1 (due to softmax)
...
def test_save_load_model(optimizer, tmp_path):
    ...
    if not hasattr(optimizer.model, "loss") or optimizer.model.loss.__name__ != portfolio_loss.__name__:
         optimizer.model.compile(optimizer='adam', loss=portfolio_loss) # Removed backslashes
    ...
    new_optimizer = PortfolioOptimizer(n_assets=N_ASSETS, lookback_window=LOOKBACK_WINDOW, hidden_units=HIDDEN_UNITS)
    ...
    assert new_optimizer.model.loss.__name__ == portfolio_loss.__name__ # Compare by name
