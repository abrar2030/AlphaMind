...
def sample_mask():
    ...
    mask = tf.tensor_scatter_nd_update(mask, [[i, 0, 0, j] for i in range(BATCH_SIZE) for j in mask_indices], tf.reshape(updates, [-1]))
...
def test_temporal_attention_block_output_shape(sample_input):
    ...
    output = block(sample_input, training=False) # Set training=False for deterministic behavior (dropout)
    assert output.shape == (BATCH_SIZE, SEQ_LENGTH, D_MODEL)
...
def test_get_positional_encoding_values():
    ...
    assert np.isclose(pos_encoding[0, 0, 0], np.sin(0 / (10000**(0/100)))) # sin(0) = 0
    assert np.isclose(pos_encoding[0, 0, 1], np.cos(0 / (10000**(0/100)))) # cos(0) = 1
