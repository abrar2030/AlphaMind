...
def sample_news_data():
    ...
    dates = pd.to_datetime(pd.date_range(start='2023-01-01', periods=50, freq='12h')) # Use 'h' instead of 'H'
...
def strategy(analyzer, sample_price_data):
    ...
    analyzer.get_sentiment_score = lambda texts: np.random.rand(len(texts)) * 2 - 1 # Random scores [-1, 1]
    return SentimentBasedStrategy(analyzer, sample_price_data.copy()) # Use a copy to avoid modifying fixture
...
def test_strategy_init(strategy, analyzer, sample_price_data):
    ...
    pd.testing.assert_frame_equal(strategy.price_data.reset_index(drop=False), sample_price_data.reset_index(drop=False))
