...
def app_context():
    ...
    auth = AuthenticationSystem(app, app.config["SECRET_KEY"], token_expiration=1) # 1 second expiration
...
def test_register_existing_user(client):
    ...
    client.post("/api/auth/register", json={"username": "testuser", "password": "password123"}) # Register first
    response = client.post("/api/auth/register", json={"username": "testuser", "password": "anotherpassword"})
...
def test_token_required_decorator_success(client, auth_system):
    ...
    client.post("/api/auth/register", json={"username": "protecteduser", "password": "password123"})
    ...
    assert response.json["user"] == "protecteduser" # Check if username is passed correctly
...
def test_token_required_decorator_expired(client, auth_system):
    ...
    client.post("/api/auth/register", json={"username": "expireduser", "password": "password123"})
    login_response = client.post("/api/auth/login", json={
