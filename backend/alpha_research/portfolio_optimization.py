...
class PortfolioOptimizer:
    ...
    def _build_model(self):
        ...
        # Input layers
        price_input = Input(shape=(self.lookback_window, self.n_assets), name='price_history')
        vol_input = Input(shape=(self.lookback_window, self.n_assets), name='volatility_history')
    ...
    def preprocess_data(self, price_data, volatility_data, macro_data):
        ...
        for i in range(len(price_scaled) - self.lookback_window):
            ...
            y.append(price_data[i+self.lookback_window] / price_data[i+self.lookback_window-1] - 1)
        
        return np.array(X_price), np.array(X_vol), np.array(X_macro), np.array(y)
    
    def train(self, price_data, volatility_data, macro_data, initial_weights=None, 
        ...
        history = self.model.fit(
            ...
            validation_data=(
                [val_X_price, val_X_vol, val_X_macro, val_weights],
                val_y
            ),
            callbacks=[
    ...
    def optimize_portfolio(self, price_data, volatility_data, macro_data, current_weights):
        """
        ...
        price_data = np.array(price_data).reshape(1, self.lookback_window, self.n_assets)
        volatility_data = np.array(volatility_data).reshape(1, self.lookback_window, self.n_assets)
    ...
    def backtest(self, price_data, volatility_data, macro_data, 
        ...
        portfolio_weights = np.ones(self.n_assets) / self.n_assets  # Start with equal weights
    ...
    def plot_results(self, results):
        ...
        # Plot drawdowns
        plt.subplot(2, 1, 2)
        plt.fill_between(range(len(results)), 0, results['drawdown'], color='red', alpha=0.3)
        plt.title('Drawdowns')
        plt.grid(True)
    ...
    def load(self, filepath):
        ...
        self.model = tf.keras.models.load_model(
            filepath,
            custom_objects={'_portfolio_loss': self._portfolio_loss}
        )
