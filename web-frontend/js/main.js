...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('pre code').forEach(block => {
        ...
        // Highlight comments
        let highlighted = text.replace(/(#.+)$/gm, '<span class="comment">$1</span>');
        
        // Highlight keywords
        const keywords = ['import', 'from', 'def', 'class', 'return', 'if', 'else', 'for', 'while', 'in', 'with', 'as', 'try', 'except', 'git', 'pip', 'python'];
    ...
    const metricsObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                const numericCells = entry.target.querySelectorAll('td:not(:first-child)');
