...
class TimeSeriesDiscriminator(tf.keras.layers.Layer):
    def __init__(self, seq_length):
        super().__init__()
        self.seq_length = seq_length
        self.conv1 = tf.keras.layers.Conv1D(64, kernel_size=3, strides=2, padding='same')
        self.conv2 = tf.keras.layers.Conv1D(128, kernel_size=3, strides=2, padding='same')
...
def regime_consistency_loss(regime_match):
    ...
    kl_div = tf.reduce_sum(expected_distribution * tf.math.log(expected_distribution / regime_match))
    return kl_div
